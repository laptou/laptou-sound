	...
	async queue(
		...
		for (const message of batch.messages) {
			try {
				...
				console.error(
					`Processing failed for ${message.body.versionId}:`,
					error,
				);

				// update status to failed using Drizzle
				const db = drizzle(env.DB, { schema });
				await updateTrackVersionStatus(db, message.body.versionId, "failed");
...
async function _transcodeToMp3(input: ArrayBuffer): Promise<ArrayBuffer> {
	// in production:
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function _analyzeWaveform(audioData: ArrayBuffer): Promise<WaveformData> {
