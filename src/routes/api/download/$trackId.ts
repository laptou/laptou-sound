...
export const APIRoute = createAPIFileRoute("/api/download/$trackId")({
  GET: async ({ params, request, context }) => {
    ...
    if (!track.is_downloadable) {
      return Response.json({ error: "Downloads not enabled for this track" }, { status: 403 });
    }

    if (!track.original_key) {
      return Response.json({ error: "No downloadable file available" }, { status: 404 });
    }
