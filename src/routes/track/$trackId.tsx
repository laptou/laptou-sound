...
export const Route = createFileRoute("/track/$trackId")({
	head: ({ loaderData }) => ({
		meta: [
			...
			{
				name: "description",
				content: loaderData?.description || "Listen on laptou sound",
			},
		],
	}),
	loader: async ({ params }) => {
				...
				<div class="flex flex-col md:flex-row gap-8 mb-8">
					...
					<div class="flex-1 min-w-0">
						...
						<Show when={track()?.processing_status !== "complete"}>
							<div class="mb-6">
								<ProcessingStatus
									status={track()?.processing_status ?? "pending"}
								/>
								<p class="text-small mt-2">
									This track is still being processed. Playback will be
									available soon.
								</p>
							</div>
						</Show>
